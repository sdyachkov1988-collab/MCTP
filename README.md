...
Подтвержденная стадия репозитория: `v2.0-step2-fix` (pending acceptance).
...
## Что не реализовано
- `v2.0` backtest wiring completion (`--strategy` flag in `run_backtest_csv.py`, OCO wiring in `_run_v20_btcusdt_mtf`)
...
These artifacts document the current accepted testnet/pre-live behavior only. They do not claim live-trading support or v2.0 automation.
